...
def autopad(k, p=None, d=1):  
    ...
    # Automatically padding the input feature layer according to the Same principle
    if d > 1:
        # actual kernel-size
...
class C2f(nn.Module):
    ...
    def forward(self, x):
        # Perform a convolution, then split into two parts, each with channel c
        y = list(self.cv1(x).split((self.c, self.c), 1))
        # After each residual structure, stack them together, dense residual
        y.extend(m(y[-1]) for m in self.m)
        return self.cv2(torch.cat(y, 1))
    
class SPPF(nn.Module):
    # SPP structure, max pooling with 5, 9, 13 kernel sizes
...
class Backbone(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(x.shape)
        #-----------------------------------------------#
        #   Output of dark3 is 256, 80, 80, an effective feature layer
        #-----------------------------------------------#
        x = self.dark3(x)
        # print(x.shape)
        x = self.am2(x)
        ...
        feat1 = x
        #-----------------------------------------------#
        #   Output of dark4 is 512, 40, 40, an effective feature layer
        #-----------------------------------------------#
        x = self.dark4(x)
        x = self.am3(x)
        feat2 = x
        #-----------------------------------------------#
        #   Output of dark5 is 1024 * deep_mul, 20, 20, an effective feature layer
        #-----------------------------------------------#
        x = self.dark5(x)
        feat3 = x
