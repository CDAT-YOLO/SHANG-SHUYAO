...
def autopad(k, p=None, d=1):  
    ...
    # Automatically pad the input feature map according to the Same principle
    if d > 1:
        # actual kernel-size
...
class C2f(nn.Module):
    ...
    def forward(self, x):
        ...
        # Retain the result after each residual structure and stack them together, dense residual
        y.extend(m(y[-1]) for m in self.m)
        return self.cv2(torch.cat(y, 1))
    
class SPPF(nn.Module):
    # SPP structure, max pooling with kernels of 5, 9, 13.
